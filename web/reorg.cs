...
namespace GeneXus.Programs {
   public class reorg : GXReorganization
   {
      ...
      {
         string cmdBuffer = "";
         /* Indices for table Veiculo */
         try
         {
            cmdBuffer=" CREATE TABLE [Veiculo] ([VeiculoId] smallint NOT NULL , [VeiculoPlaca] nchar(20) NOT NULL , [VeiculoChassi] nchar(20) NOT NULL , [ModeloId] smallint NOT NULL , PRIMARY KEY([VeiculoId]))  "
            ;
         ...
         catch
         {
            try
            {
               DropTableConstraints( "[Veiculo]") ;
               cmdBuffer=" DROP TABLE [Veiculo] "
               ;
               RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
               RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
               RGZ.ExecuteStmt() ;
            ...
            catch
            {
               ...
               catch
               {
                  ...
                  {
                     DropTableConstraints( "[Veiculo]") ;
                     cmdBuffer=" DROP FUNCTION [Veiculo] "
                     ;
                     RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
         ...
         try
         {
            cmdBuffer=" CREATE NONCLUSTERED INDEX [IVEICULO1] ON [Veiculo] ([ModeloId] ) "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
         ...
         {
            cmdBuffer=" DROP INDEX [IVEICULO1] ON [Veiculo] "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_MASKNOTFOUND | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
      ...
      public void RIVeiculoModelo( )
      {
         string cmdBuffer;
         try
         {
            cmdBuffer=" ALTER TABLE [Veiculo] ADD CONSTRAINT [IVEICULO1] FOREIGN KEY ([ModeloId]) REFERENCES [Modelo] ([ModeloId]) "
            ;
            RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
            RGZ.ErrorMask = GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK;
            RGZ.ExecuteStmt() ;
         ...
         catch
         {
            try
            {
               cmdBuffer=" ALTER TABLE [Veiculo] DROP CONSTRAINT [IVEICULO1] "
               ;
               RGZ = new GxCommand(dsDefault.Db, cmdBuffer, dsDefault,0,true,false,null);
               RGZ.ErrorMask = GxErrorMask.GX_MASKNOTFOUND | GxErrorMask.GX_MASKLOOPLOCK;
               RGZ.ExecuteStmt() ;
      ...
      private bool PreviousCheck( )
      {
         ...
         if ( tableexist("Veiculo",sSchemaVar) )
         {
            SetCheckError ( GXResourceManager.GetMessage("GXM_table_exist", new   object[]  {"Veiculo"}) ) ;
            return false ;
         }
         return true ;
      }
      ...
      {
         ReorgExecute.RegisterBlockForSubmit( 1 ,  "CreateVeiculo" , new Object[]{ });
      }

      private void ExecuteOnlyRisReorganization( )
      {
         ReorgExecute.RegisterBlockForSubmit( 2 ,  "RIVeiculoModelo" , new Object[]{ });
      }
      ...
      {
         GXReorganization.SetMsg( 1 ,  GXResourceManager.GetMessage("GXM_filecrea", new   object[]  {"Veiculo", ""}) );
      }

      private void SetPrecedenceris( )
      {
         GXReorganization.SetMsg( 2 ,  GXResourceManager.GetMessage("GXM_refintcrea", new   object[]  {"[IVEICULO1]"}) );
         ReorgExecute.RegisterPrecedence( "RIVeiculoModelo" ,  "CreateVeiculo" );
      }
